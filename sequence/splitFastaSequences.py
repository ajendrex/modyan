...
def main():
  ...
  commandLineParser.add_argument('-s', '--separator', dest="seps", default=['n'], nargs="+", help='Separators to be used.')
  ...
  for record in SeqIO.parse(inputHandle, "fasta"):
    i = 1
    seqName = record.id
    for subseq in re.split('[' + '|'.join(options.seps) + ']+', str(record.seq)):
